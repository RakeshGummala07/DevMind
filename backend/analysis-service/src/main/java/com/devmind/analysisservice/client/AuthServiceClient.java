        ...
        try {
            ...
            if (status.value() == 401) {
                log.error("auth-service rejected the internal API key — INTERNAL_API_KEY likely " +
                        "doesn't match between analysis-service and auth-service");
            ...
            if (status.value() == 400) {
                throw AppException.badRequest("GITHUB_NOT_CONNECTED", "Connect a GitHub account before requesting a PR review");
            }
